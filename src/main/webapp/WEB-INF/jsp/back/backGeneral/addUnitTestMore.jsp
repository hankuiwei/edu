...
<script src="${pageContext.request.contextPath}/jsp/back/js/jquery.cookie.js"></script>
    ...
    <form method="post" class="form-x" id="myform">  
      ...
      <div class="form-group">
       ...
       <div class="form-group">
        ...
        <div class="field">
          <select name="vcId" id="vcId" class="w50"  style="height:36px;line-height:34px;" onchange="changeTableList(this.value)" >
             <c:forEach items="${vlist }" var="s">
              <c:if test="${s.vcid==vcId}">
              <option value="${s.vcid}" selected="selected">${s.vcname}</option>
              </c:if>
              <c:if test="${s.vcid!=vcId}">
             <option value="${s.vcid}">${s.vcname}</option>
               </c:if>
              </c:forEach>
            </select>
          <div class="tips"></div>
...
//切换章节
function changeTableList(utid){
	var type=$("#type").val();
	var isMore=$("#isMore").val();
	if(isMore==1){
		type=type==0?2:3;
	}
	$.post("${pageContext.request.contextPath}/back/getUnitTestHtml",{utid:utid,utype:type},function(res){
		res=eval("("+res+")");
		if(res.status=="ok"){
			$("#tests_table").html(res.data);
		}else{
			alert(res.data);
		}
...
$(function(){
	...
	var vcId_select=$.cookie("vcId_select");
	if(vcId_select!=null&&vcId_select.length>0&&vcId_select!='null'){
	    if(vcId_select!=$("#vcId").val()){
	    	$('#vcId option[value='+vcId_select+']').attr('selected',true);
	    	changeTableList(vcId_select);
	    }
		$.cookie("vcId_select",null);
	}
...
function toEdit(utid){
	window.open("${pageContext.request.contextPath}/back/backGeneral/updateUnitTest.html?utid="+utid+"&vcId="+$("select[name='vcId']").val(),'_self');
}
//异步上传文件
