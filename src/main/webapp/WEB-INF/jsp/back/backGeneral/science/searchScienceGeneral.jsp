...
<body>
  <div class="panel admin-panel">
    ...
    <form method="get" action="back/backGeneral/science/searchScienceGeneral" id="listform">
    <div class="padding border-bottom">
      <ul class="search" style="padding-left:10px;">
        <li> <a class="button border-main icon-plus-square-o" href="${pageContext.request.contextPath}/back/backGeneral/science/addScienceGeneral"> 添加内容</a> 
            <a class="button border-main icon-plus-square-o" href="${pageContext.request.contextPath}/back/backGeneral/addVideoMore.html?type=3"> 添加专题</a>
        </li>
         <li>
            <select name="gclass" style="height:40px;line-height:38px;margin-top:1px;width:200px;">
              ...
              <option value="六年级" <c:if test="${map['gclass']=='六年级' }">selected="selected"</c:if>>六年级</option>
            ...
            <select name="gclassify" style="height:40px;line-height:38px;margin-top:1px;width:200px;" >
              ...
              <option value="专题课" <c:if test="${map['gclassify']=='专题课' }">selected="selected"</c:if>>专题课</option>
              <option value="长期班" <c:if test="${map['gclassify']=='长期班' }">selected="selected"</c:if>>长期班</option>
              <option value="衔接班" <c:if test="${map['gclassify']=='衔接班' }">selected="selected"</c:if>>衔接班</option>
              <option value="入学考" <c:if test="${map['gclassify']=='入学考' }">selected="selected"</c:if>>入学考</option>
              <option value="分班考" <c:if test="${map['gclassify']=='分班考' }">selected="selected"</c:if>>分班考</option>
            </select>
          </li>
        <li>
...
$(function(){
	...
	window.location.href="${pageContext.request.contextPath}/back/deleteScienceGeneralByid?gid="+gid+"&isMore="+isMore;		
	});
	//更新
	$(".border-up").on("click",function(){
		...
		if(isMore==1){//多关联
			...
			window.location.href="${pageContext.request.contextPath}/back/backGeneral/science/updateScienceGeneral?gid="+gid;
		}
		
	});
})
function pageSize(num){
	window.location.href="${pageContext.request.contextPath}/back/backGeneral/science/searchScienceGeneral?gclass="
		+"${map['gclass']}"+"&"+"gclassify="+"${map['gclassify']}"+"&"+"pageNo="+num;
}
//添加单元/章节测试题		
function addUnitTest(vid,vname,isMore){
  if(isMore&&isMore==1){//专题多集
	  window.location.href="${pageContext.request.contextPath}/back/backGeneral/addUnitTestMore.html?vid="+vid+"&type=0&vname="+vname+"&typeName=科学";
  }else{//内容单集
	  window.location.href="${pageContext.request.contextPath}/back/backGeneral/addUnitTest.html?vid="+vid+"&type=0&vname="+vname+"&typeName=科学";
  }
}	
</script>
</body>
