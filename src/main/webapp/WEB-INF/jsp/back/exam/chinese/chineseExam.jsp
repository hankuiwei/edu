...
<body>
  <div class="panel admin-panel">
    ...
    <form method="get" action="back/exam/chinese/chineseExam.html" id="listform">
    <div class="padding border-bottom">
      <ul class="search" style="padding-left:10px;">
        <%-- <li> <a class="button border-main icon-plus-square-o" href="${pageContext.request.contextPath}/back/exam/chinese/toAddChineseExam.html"> 添加内容</a> </li> --%>
        <li> <a class="button border-main icon-plus-square-o" href="${pageContext.request.contextPath}/back/exam/toAddExam.html?subject=语文测验"> 添加内容</a> </li>
        <li>
            ...
            <select name="etclassify" style="height:40px;line-height:38px;margin-top:1px;width:200px;" >
             ...
             <%--  <option value="上学期" <c:if test="${map['etclassify']=='上学期' }">selected="selected"</c:if>>上学期</option>
              ...
              <option value="上/下学期" <c:if test="${map['etclassify']=='上/下学期' }">selected="selected"</c:if>>上/下学期</option>
            </select>
          </li>
        <li>
...
$(function(){
	...
	window.location.href="${pageContext.request.contextPath}/back/deleteExamById?gid="+gid;	
	});
	$(".border-up").on("click",function(){
		var gid = $(this).siblings(".gid").val();
		//window.location.href="${pageContext.request.contextPath}/back/exam/chinese/updateChinexeExam.html?gid="+gid;
		window.location.href="${pageContext.request.contextPath}/back/exam/updateExam.html?gid="+gid;
	})
})
function pageSize(num){
window.location.href="${pageContext.request.contextPath}/back/exam/chinese/chineseExam.html?etclass="+"${map['etclass']}"+"&"+"etclassify="+"${map['etclassify']}"+"&"+"pageNo="+num;
		}
//添加单元/章节测试题		
function addUnitTest(vid,vname){
 window.location.href="${pageContext.request.contextPath}/back/backGeneral/addUnitTest.html?vid="+vid+"&type=4&vname="+vname+"&typeName=语文测验";
}
//添加试卷
