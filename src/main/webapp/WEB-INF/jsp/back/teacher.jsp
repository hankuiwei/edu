...
$(function(){
	...
	$('.border-red').on('click',function(){
		var id = $(this).siblings(".tech").val();
	window.location.href="${pageContext.request.contextPath}/back/deleteTeacher?id="+id;
	});
	...
	/* $(".border-up4").on("click",function(){
		var id = $(this).siblings(".tech").val();
		var tname=$("#tname_"+id).html();
		window.location.href="${pageContext.request.contextPath}/back/editUnitTestScores.html?tname="+tname;
	}); */
});
//上移/下移调整顺序
function changeSort(type,id){
	 ...
	 if(change_tr_id){
		 ...
		 //修改库顺序
		 $.post("${pageContext.request.contextPath}/back/updateTeacherSort",{id1:id,sort1:new_sort,id2:change_id,sort2:cur_sort},function(res){
