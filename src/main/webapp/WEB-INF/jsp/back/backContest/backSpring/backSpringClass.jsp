...
<body>
  <div class="panel admin-panel">
    <div class="panel-head"><strong class="icon-reorder">迎春杯课程列表</strong> <a href="" style="float:right; display:none;">添加字段</a></div>
    <form method="get" action="back/backContest/backSpring/backSpringClass.html" id="listform">
    <div class="padding border-bottom">
      <ul class="search" style="padding-left:10px;">
        <li> <a class="button border-main icon-plus-square-o" href="${pageContext.request.contextPath}/back/backContest/backSpring/toAddSpringClass.html"> 添加内容</a> </li>
         <li>
            <select name="cclass" style="height:40px;line-height:38px;margin-top:1px;width:200px;" >
              <option value="" >--请选择类别--</option>
              <option value="三年级" <c:if test="${map['cclass']=='三年级' }">selected="selected"</c:if>>三年级</option>
              <option value="四年级" <c:if test="${map['cclass']=='四年级' }">selected="selected"</c:if>>四年级</option>
              <option value="五年级" <c:if test="${map['cclass']=='五年级' }">selected="selected"</c:if>>五年级</option>
              <option value="六年级" <c:if test="${map['cclass']=='六年级' }">selected="selected"</c:if>>六年级</option>
            </select>
          </li>
              <li>
            <select name="cclassify" style="height:40px;line-height:38px;margin-top:1px;width:200px;" >
              <option value="">--请选择分类--</option>
              <option value="历史真题" <c:if test="${map['cclassify']=='历史真题' }">selected="selected"</c:if>>历史真题</option>
...
$(function(){
	...
	window.location.href="${pageContext.request.contextPath}/back/deleteSpringCLass?cid="+cid;	
	});
	$(".border-up").on("click",function(){
		var cid = $(this).siblings(".cid").val();
		window.location.href="${pageContext.request.contextPath}/back/backContest/backSpring/toUpdateSpringClass.html?cid="+cid;
	})
})
function pageSize(num){
window.location.href="${pageContext.request.contextPath}/back/backContest/backSpring/backSpringClass.html?cclass="+"${map['cclass']}"+"&"+"cclassify="+"${map['cclassify']}"+"&"+"pageNo="+num;
		}
//添加单元/章节测试题		
function addUnitTest(vid,vname,isMore){
  if(isMore&&isMore==1){//专题多集
	  window.location.href="${pageContext.request.contextPath}/back/backGeneral/addUnitTestMore.html?vid="+vid+"&type=1&vname="+vname+"&typeName=迎春杯";
  }else{//内容单集
	  window.location.href="${pageContext.request.contextPath}/back/backGeneral/addUnitTest.html?vid="+vid+"&type=1&vname="+vname+"&typeName=迎春杯";
  }
}		
