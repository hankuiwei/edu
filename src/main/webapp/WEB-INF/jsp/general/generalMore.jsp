...
<base href="${pageContext.request.contextPath}/jsp">
		...
		<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/jsp/css/index.css?v=1"/>
	...
	<script src="${pageContext.request.contextPath}/jsp/js/main.js" type="text/javascript"></script>
...
<!-- logo图 和搜素-->
		...
		<!-- 视频 -->
		<div class="content"  >
			<div class="content-video" style="background:url('${pageContext.request.contextPath}/jsp/img/bg23.png') ">
			<table style="width:100%;height:577px;">
			<tr><td style="height:577px;"><div id="myvideo" style="width:100%;height:100%;">Loading the player...</div></td>
			...
			<ul class="ce">
			  <li> 
				   ...
				   <c:forEach items="${generalVideo.vclist}" var="gl" varStatus="st">
				    <c:if test="${st.index==0 }">
							<li id="vli_${st.index}"  class="choose_li"><a href="javascript:void(0)"  style="color:black;" onclick="playVideo('${gl.vcideoUrl}',${st.index},'${gl.vcid}','${gl.vcname}',true)">${gl.vcname}</a></li>
					</c:if>		
					 <c:if test="${st.index>0 }">
							<li id="vli_${st.index}" ><a href="javascript:void(0)"  onclick="playVideo('${gl.vcideoUrl}',${st.index},'${gl.vcid}','${gl.vcname}',true)">${gl.vcname}</a>    </li>
					</c:if>
					 </c:forEach>		
				   </ul>
			  </li>
			  <li> 
				   ...
				   <ul class="er2" style="display:block;">
							<li><a  href="javascript:void(0)" id="testName" onclick="openUnitTest()">${videoChild.vcname}测验题</a>  </li>
				   </ul>
			  </li>
			  <li> 
    ...
    <div class="modal-dialog" style="width:850px;" >
        <div class="modal-content" style="width:850px;">
			...
			function playVideo(url,index,vcid,vcname,isplay){
				changeVideoPlayNum();
				//修改背景
				/* if(index!=playIndex){
					$("#vli_"+playIndex).css("background","");
					$("#vli_"+index).css("background","#5CB85C");
				} */
				if(!isRemove){
				...
				//切换视频播放
				initPlayer(playerInstance,url,isplay);
				playIndex=index;
				//修改章节测试题
				vcId=vcid;
				vcname=vcname;
...
//						暂停当前播放视频
		  		    	...
		  		    	// alert("code=============="+code);
		  		    	  ...
		  		    	  if(code.length>0){
					  		 ...
					  		 var gname="${generalVideo.gname}";
					  		  if(gclass.indexOf("古诗")>-1||gclass.indexOf("阅读")>-1||gclass.indexOf("写作")>-1||gclass.indexOf("语法")>-1||gclass.indexOf("流利英语")>-1||gclass.indexOf("其他")>-1){
					  			gclass="";
					  		 } 
					  		 if(gname.length>0){
					  			gname="【"+gname+"】";
				...
				<!-- 原价 -->
				 <c:if test="${killInfo==null||killInfo.timeType==2||generalVideo.isKill==0}">
						...
						<div class="classification-content video-chapter2" style="height:70px;line-height:70px;padding-top:1px;margin:0px;background:#E7442E;width:95%;">
							     ...
							     <table style="width:95%;"><tr><td style="color:white;font-weight:bold;padding-left:10px;">￥${generalVideo.gmoney}</td>
							     <td>
							     <c:if test="${isBuy==0}">
				 ...
				 <c:if test="${killInfo!=null&&killInfo.timeType!=2&&generalVideo.isKill==1}">
						...
						<div class="classification-content video-chapter2" style="height:70px;line-height:70px;padding-top:1px;margin:0px;background:#E7442E;">
							     ...
							     <table style="width:95%;"><tr>
							      <td style="color:white;font-weight:bold;padding-left:10px;width:100px;">￥${generalVideo.killMoney}</td>
							       <td style="color:white;padding-left:5px;">专柜价：<a style='text-decoration:line-through;color:white;'>￥${generalVideo.gmoney}</a></td>
							     <td>
							      <c:if test="${killInfo.timeType==1 }">
								       <c:if test="${isBuy==0}">
									      <div class="user-submit" style="height:60px;" >
	...
	function initUnitTests(vid,type,isRedo,vname){
		...
		vname=vname?vname:vcname;
		 //加载章节测试题
		  $.post("${pageContext.request.contextPath}/back/getUnitTestsHtmlByVid",{vid:vid,utype:type,isRedo:isRedo,vname:"${generalVideo.gname}"+vname},function(res){
			  res=eval("("+res+")");
			  if(res.status=="ok"){
			 ...
			 setTimeout(function(){
			   $.post("${pageContext.request.contextPath}/general/getLastNewVideo",{gid:"${generalVideo.gid}",lastVcid:"${lastVcid}"},function(data){
				  if(data){
					  ...
					  var li_html="";
					  if(li_size==0){
						  li_html="<li id=\"vli_0\"  class=\"choose_li\"><a href=\"javascript:void(0)\"  style=\"color:black;\" onclick=\"playVideo('"+data.vcideoUrl+"',0,'"+data.vcid+"','"+data.vcname+"',true)\">"+data.vcname+"</a></li>";
						  playVideo(data.vcideoUrl,0,data.vcid,data.vcname,false);
					  }else{
						  li_html="<li id=\"vli_"+li_size+"\" ><a href=\"javascript:void(0)\"  onclick=\"playVideo('"+data.vcideoUrl+"',"+li_size+",'"+data.vcid+"','"+data.vcname+"',true)\">"+data.vcname+"</a></li>";
					  }
					  $(".er").append(li_html);
				  }
			   });
