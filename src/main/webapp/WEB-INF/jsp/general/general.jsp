			  ...
			  <li> 
				 ...
				 <ul class="er" style="display:block;">
				   <li class="choose_li" style="word-break:break-all;word-wrap:break-word;">
				   	<a href="javascript:void(0)"  style="color:black;" onclick="playVideo('${generalVideo.gvideoUrl}')">${generalVideo.gname}</a>
				   </li>	
				</ul>
			  </li>
			  <li> 
				   ...
				   <ul class="er2" style="display:block;">
							<li><a  href="javascript:void(0)" id="testName" onclick="openUnitTest()">本章测验题</a>  </li>
				   </ul>
			  </li>
			  <li> 
...
//						暂停当前播放视频
	...
	function initUnitTests(vid,type,isRedo){
		isRedo=isRedo?1:0;
		 //加载章节测试题
		  $.post("${pageContext.request.contextPath}/back/getUnitTestsHtmlByVid",{vid:vid,utype:type,isRedo:isRedo,vname:"${generalVideo.gname}"},function(res){
			  res=eval("("+res+")");
			  if(res.status=="ok"){
	...
	function submitAnswer(){
		...
		$("#result_tests").show();
		//将学生成绩入库
		$.post("${pageContext.request.contextPath}/back/addUserScore",{vid:vid,utype:0,score:totalScore,rightNum:right_num,errorNum:error_num,
